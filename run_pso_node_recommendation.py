...
RAW_LOG = Path("outputs/data/csv/raw/pso_node_recommendation_convergence_50iter.csv")
SUMMARY_LOG = Path("outputs/data/csv/summary/pso_node_recommendation_summary_50iter.csv")
FIGURE_PATH = Path("outputs/figures/report_node_set_smooth/pso_convergence_node_recommendation_50iter.png")
...
SUMMARY_FIELDS = [
    ...
    "runtime_mean_sec",
    "relative_iter_2_mean",
    "relative_iter_5_mean",
...
def _normalize_convergence(events: List[Dict[str, float]]) -> List[Dict[str, float]]:
    ...
    for event in sorted(events, key=lambda row: int(row["iteration"])):
        ...
        if initial is None or initial <= 0 or not math.isfinite(score):
            ...
            relative = round(score / initial, 8)
        normalized.append({**event, "relative_best_score": relative})
    return normalized


def _run_one(args: Dict[str, int]) -> List[Dict[str, object]]:
    ...
    case = replace(
        SIMULATION_CASE,
        name=f"pso_node_recommendation_s{space_m}_n{node_count}_seed{seed}",
...
def build_summary(raw_rows: List[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    for key, rows in sorted(by_case.items(), key=lambda item: (int(item[0][0]), int(item[0][1]))):
        space_m, node_count = int(key[0]), int(key[1])
        ...
        summary_rows.append(
            {
                ...
                "relative_iter_2_mean": round(rel_2, 4),
                "relative_iter_5_mean": round(rel_5, 4),
                "relative_iter_10_mean": round(rel_10, 4),
                "relative_iter_50_mean": round(rel_50, 4),
                "drop_iter_2_pct": round((1.0 - rel_2) * 100.0, 2),
                "drop_iter_5_pct": round((1.0 - rel_5) * 100.0, 2),
...
def plot(raw_rows: List[Dict[str, object]], summary_rows: List[Dict[str, object]]) -> None:
    ...
    for ax, space_m in zip(axes, [100, 500, 1000]):
        for node_count in NODE_SETS[space_m]:
            ...
            ax.plot(iterations, means, marker="o", markersize=3, linewidth=1.7, label=f"n={node_count}")
    ...
    detail = {int(row["space_m"]): [] for row in summary_rows}
    for row in summary_rows:
        detail[int(row["space_m"])].append(
            f"n={int(row['node_count'])}: d10={float(row['drop_iter_10_pct']):.1f}%"
        )
    fig.suptitle("PSO convergence search by node count, first round, 50 iterations, 10 seeds/case")
