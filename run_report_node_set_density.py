...
RAW_LOG = Path("outputs/data/csv/raw/report_node_set_density_runs.csv")
CONVERGENCE_LOG = Path("outputs/data/csv/raw/report_node_set_density_convergence_events.csv")
SUMMARY_LOG = Path("outputs/data/csv/summary/report_node_set_density_summary.csv")
FIGURE_DIR = Path("outputs/figures/report_node_set_density")
...
def _existing_keys() -> set[Tuple[str, str, str, str, str, str]]:
    return {
        ...
        if row.get("suite") == "REPORT_NODE_SET_DENSITY"
    }
...
def _run_one_case(args: Dict[str, object]) -> Tuple[Dict[str, object], List[Dict[str, object]]]:
    ...
    case = replace(
        SIMULATION_CASE,
        name=f"report_density_s{space_m}_n{node_count}_seed{seed}",
...
def build_summary() -> List[Dict[str, object]]:
    ...
    grouped = _group_rows(
        raw_rows,
        ("space_m", "node_count", "contention_penalty_factor", "pso_particles", "pso_iterations"),
    )
...
def _bar_panels(
    ...
    *,
    ...
    output_name: str,
    tick_step: float,
    label_kind: str = "value",
) -> None:
    import matplotlib.pyplot as plt

    max_value = max(
        float(row[value_field]) + (float(row[std_field] or 0) if std_field else 0.0)
    ...
    for ax, space_m in zip(axes, REPORT_CASES):
        ...
        stds = [float(row[std_field] or 0) for row in panel] if std_field else [0.0 for _ in panel]
        positions = list(range(len(labels)))
        bars = ax.bar(positions, means, yerr=stds if std_field else None, capsize=4, color="#4C78A8", alpha=0.84, width=0.66)
        for bar, mean_value, std_value, row in zip(bars, means, stds, panel):
            if label_kind == "cv":
                label = f"CV={mean_value:.1f}%\nstd={float(row['ft5_std']):.0f}"
            else:
...
def plot_ft5(summary_rows: List[Dict[str, object]]) -> None:
    _bar_panels(
        ...
        std_field="ft5_std",
        ylabel="FT5 round",
        title="FT5 across selected node sets, density/contention model",
        output_name="report_node_set_ft5_bar.png",
        tick_step=250,
    )
...
def plot_ft5_cv(summary_rows: List[Dict[str, object]]) -> None:
    _bar_panels(
        summary_rows,
        value_field="ft5_cv_pct",
        std_field=None,
        ylabel="FT5 coefficient of variation (%)",
        title="FT5 stability across selected node sets, density/contention model",
        output_name="report_node_set_ft5_cv_std_bar.png",
        tick_step=2.5,
        label_kind="cv",
    )


def plot_relative_ft5(summary_rows: List[Dict[str, object]]) -> None:
    import matplotlib.pyplot as plt
    ...
    for ax, space_m in zip(axes, REPORT_CASES):
        ...
        positions = list(range(len(labels)))
        bars = ax.bar(positions, rel_values, color="#4C78A8", alpha=0.84, width=0.66)
    ...
    fig.suptitle("Relative FT5 trend within each fixed space, selected node sets")
    output_path = FIGURE_DIR / "report_node_set_ft5_relative_by_space.png"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_path, dpi=160, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved {output_path}")
...
def plot_convergence(summary_rows: List[Dict[str, object]]) -> None:
    ...
    for ax, (space_m, node_counts) in zip(axes, REPORT_CASES.items()):
        for node_count in node_counts:
            ...
            ax.plot(iterations, means, marker="o", markersize=3, linewidth=1.5, label=f"n={node_count}")
    ...
    output_path = FIGURE_DIR / "report_node_set_pso_convergence_by_node_count.png"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_path, dpi=160, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved {output_path}")
...
def plot_ft5_vs_neighbor(summary_rows: List[Dict[str, object]]) -> None:
    import matplotlib.pyplot as plt

    fig, ax = plt.subplots(figsize=(10.5, 5.8), constrained_layout=True)
    for space_m in REPORT_CASES:
        panel = [row for row in summary_rows if int(row["space_m"]) == space_m]
        panel.sort(key=lambda row: float(row["avg_neighbor_count_mean"]))
        xs = [float(row["avg_neighbor_count_mean"]) for row in panel]
        ys = [float(row["ft5_mean"]) for row in panel]
        ax.plot(xs, ys, marker="o", linewidth=1.8, label=f"space={space_m}m")
        for row, x_value, y_value in zip(panel, xs, ys):
            ax.text(x_value, y_value + 18, f"n={row['node_count']}", ha="center", va="bottom", fontsize=8)

    ax.set_title("FT5 vs average neighbor count, selected node sets")
    ax.set_xlabel("Average neighbors within transmission range")
    ax.set_ylabel("FT5 round")
    ax.set_ylim(0, _nice_upper(max(float(row["ft5_mean"]) for row in summary_rows), 250))
    ax.grid(True, linestyle="--", alpha=0.35)
    ax.legend(loc="best", framealpha=0.92)
    output_path = FIGURE_DIR / "report_node_set_ft5_vs_avg_neighbor_count.png"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_path, dpi=160, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved {output_path}")
...
def check_monotonic_mean() -> List[str]:
    stats = _iteration_stats()
    issues: List[str] = []
    for space_m, node_counts in REPORT_CASES.items():
        for node_count in node_counts:
            iterations = sorted(
                iteration
                for (space_value, node_value, iteration) in stats
            ...
            means = [stats[(str(space_m), str(node_count), iteration)][0] for iteration in iterations]
            for prev_iteration, next_iteration, prev_mean, next_mean in zip(iterations, iterations[1:], means, means[1:]):
...
def plot_figures(summary_rows: List[Dict[str, object]]) -> None:
    ...
    plot_ft5_vs_neighbor(summary_rows)
...
def run_all(args: argparse.Namespace) -> None:
    ...
    for space_m, node_counts in REPORT_CASES.items():
        for node_count in node_counts:
            for seed in seeds:
                ...
                if key in existing:
                    print(f"Skip existing report density space={space_m}, n={node_count}, seed={seed}")
                    continue
                jobs.append(
                    {
                        "space_m": space_m,
    ...
    if jobs:
        print(f"Running {len(jobs)} report-density jobs with {args.workers} workers")
        ...
        with ProcessPoolExecutor(max_workers=max(1, args.workers)) as executor:
            ...
            for future in as_completed(future_to_job):
                ...
                print(
                    ...
                    f"neighbors={run_row['avg_neighbor_count']}, events={len(event_rows)}"
                )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run report node-set density/contention experiments.")
    parser.add_argument("--runs", type=int, default=10)
    parser.add_argument("--seed-start", type=int, default=BASE_SEED + 3000)
